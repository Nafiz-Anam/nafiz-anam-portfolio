...
import { Plus, Search, Edit3, Trash2, Loader2, FileText, ExternalLink } from "lucide-react";
import { type BlogListItem } from "@portfolio/types";
import { api } from "@/lib/api";

type AdminPost = BlogListItem & { updatedAt: Date | string };
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and publish insights articles.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex h-60 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-border text-center">
          ...
          <Link href="/blog/new" className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-slate-700">
            <Plus size={14} /> New post
          </Link>
        </div>
      ) : (
                ...
                <tr key={p.id} className="border-b border-border/60 last:border-b-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      {p.status === "published" && (
                        <a
                          href={`${WEBSITE_URL}/insights/${p.slug}`}
                          target="_blank"
                          rel="noreferrer"
